...
class LNWallet(LNWorker):
        ...
        while True:
            ...
            if amount_to_send > 0:
                ...
                if self.channel_db:
                    ...
                    route, self.trampoline2 = await self.create_trampoline_route(
                        amount_to_send, node_pubkey, invoice_features, r_tags, t_tags,
                        self.trampoline_fee_level, self.trampoline2_list)
    ...
    async def create_trampoline_route(
            self, amount_msat:int, invoice_pubkey:bytes, invoice_features:int, r_tags, t_tags,
            trampoline_fee_level:int, trampoline2_list:List[bytes]) -> LNPaymentRoute:
